...
$dataFor=array("librarylink_link","librarylink_log","librarylink_collection");
$tableFor=array("librarylink_link","librarylink_log","librarylink_collection");
$indicesFor=array("librarylink_link","librarylink_log","librarylink_collection");
...
if (getval("execute","")!="" && enforcePostRequest(false))
	...
	for ($n=0;$n<count($tables);$n++)
		...
		{
			$f=fopen("../../plugins/librarylink/dbstruct/table_" . $table . ".txt","w");
		...
		if ($createIndices && (in_array($table,$indicesFor) || count($indicesFor)===0))
			{
			$f=fopen("../../plugins/librarylink/dbstruct/index_" . $table . ".txt","w");
		...
		if ($createData && (in_array($table,$dataFor) || count($dataFor)===0))
			{
			$f=fopen("../../plugins/librarylink/dbstruct/data_" . $table . ".txt","w");
